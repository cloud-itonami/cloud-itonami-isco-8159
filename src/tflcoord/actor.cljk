(ns tflcoord.actor
  "TflCoordActor — the ISCO-08 8159 textile, fur and leather products
  machine operators (not elsewhere classified) plant scheduling/
  ...
  on cloud-itonami-isco-8122's platingcoord.actor.
  ...
  The unconditional invariant: the Textile, Fur and Leather Products
  Machine Operations Coordination Advisor can never directly commit an
  operating record the TflCoordGovernor refuses, and can never make
  this actor finalize a machine-operation-execution decision (e.g.
  ...
  production line safe for operation), and can never override a plant
  safety officer's judgment — every commit-record! call is gated behind
  `:decide`, and that decision class is a permanent hard block
  regardless of disposition path. This actor coordinates PLANT
  SCHEDULING/LOGISTICS ONLY — it never operates textile/fur/leather
  machinery itself, and it never makes a plant-safety-clearance
  decision itself."
...
(defn build-graph
  "Build a compiled TflCoordActor graph. `store` implements
  `tflcoord.store/Store`. `advisor` implements `tflcoord.advisor/Advisor`
