(ns tflcoord.advisor
  ...
  safety concern, coordinate a raw-materials supply order) from a crew
  roster, plant registration and safety-reporting policy. Swappable
  mock/llm; the advisor ONLY proposes — `tflcoord.governor`
  ...
  (e.g. deciding to proceed with a specific textile/fur/leather machine
  run), or a plant-safety-clearance decision (e.g. declaring a plant or
  production line safe for operation), and never proposes to override a
  plant safety officer's judgment — those stay permanently out of this
  actor's scope. Modeled closely on cloud-itonami-isco-8122's
...
(defn- rationale-for [op operator-id plant-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for textile/fur/leather machine task at plant " plant-id)
...
(def ^:private system-prompt
  "You are a textile, fur and leather products machine operators (not
   elsewhere classified) plant scheduling/logistics coordination
   advisor. This is a broad residual occupation covering diverse
   ...
   without a single dominant hazard type. Given a request, propose an
   :op (one of :log-work-record, :schedule-crew-operation,
   ...
   textile/fur/leather machine run), or a plant-safety-clearance
   decision (e.g. declaring a plant or production line safe for
   operation), or to override a plant safety officer's judgment —
   those are always out of this actor's scope; it coordinates plant
   scheduling/logistics only and never operates textile/fur/leather
   machinery or clears a plant for operation itself. Safety concerns
